# Token Save Audit — сессия «проверь задачи taskagent cloud»

Дата: 2026-06-05  
Контекст: пользователь попросил проверить задачи в трекере TaskAgent Cloud,
...
|-----------|--------|----------------|
| **taskagent** | ~22 | ~120–150K символов (~30–45K токенов) |
...
|----------|------|---------|
| `taskagent_workspacegraph_search` (limit=100) | ~40% | 170 KB |
| `taskagent_search` ×2 (limit=50) | ~35% | ~50–100 KB каждый |
| `Glob **/*` репо ×2 | ~15% | тысячи путей `.next/`, `.deploy-logs/` |
| `taskagent_get` ×13 | ~8% | ~1–2 KB каждый |
| Остальное (list, plan, grep) | ~2% | — |

### Где ушло больше всего

1. **`taskagent_workspacegraph_search` (limit=100)** — 170 KB JSON.
   Результат: 0 открытых задач. Но `taskagent_list status=active` уже дал
   **1 задачу в inbox**. **~40–45K токенов впустую.**

2. **`taskagent_search` ×2** (`cloud`, `taskagent`, limit=50) — смесь задач +
   комментариев + планов. Нужен был только список **незакрытых** задач.
   **~25–35K токенов.**

3. **`Glob **/*` и `Glob **/taskagent-cloud/**`** — тысячи путей артефактов
   сборки. Для аудита не нужны. **~15–25K токенов.**

4. **`taskagent_get` ×13** — проверка задач, которые в search уже были `done`
   с комментариями аудита. **~5–8K токенов** (меньше, но лишние round-trips).

5. **Ошибочный вызов** — `taskagent_search` без `project_scope` → ambiguous
   scope. Мало токенов, но лишний hop.

6. **`codegraph_files`** на taskagent-cloud — индекс пустой, ответ бесполезен.
...
| # | Вызов | Зачем |
|---|-------|-------|
| 1 | `taskagent_workspace_info` | scope `taskagent-cloud` |
| 2 | `taskagent_list status=active project_scope=taskagent-cloud` | **1 inbox-задача** |
| 3 | `taskagent_plan_list status=completed project_scope=taskagent-cloud` | планы закрыты |
| 4 | `Grep elicit` в `/home/av/projects/taskagent-cloud` | фича не в коде |
| 5 | `taskagent_comment` на inbox-задачу | зафиксировать аудит |
...
1. **Сначала узкий list, не search** — `taskagent_list { status: "active", project_scope: "taskagent-cloud" }`.
...
5. **Всегда передавать scope с первого вызова** — `project_scope: "taskagent-cloud"`.
...
|---------------|-----------|
| `taskagent-policy.mdc` plugin vs `/home/av/projects/.cursor/rules/` | **идентичны** (diff пустой) |
| `taskagent.mdc` | **идентичны** |
| `taskagent-tasks.md` в `.cursor/commands/` | **установлен** |
...
```json
"rules": [
  { "path": "cursor/rules/taskagent-policy.mdc", ... },
  { "path": "cursor/rules/taskagent.mdc", ... }
]
```
...
**Always-applied** `taskagent-policy.mdc` (§7):

> «use `taskagent_search` for targeted lookups»

**On-demand** `taskagent.mdc` (§ Listing):

> «**Prefer search over bulk list** when the user names a topic, plan phase,
> or keyword»

Запрос: *«в трекере есть taskagent cloud…»* — агент воспринял **«taskagent
cloud» как keyword** → `taskagent_search limit=50` ×2.

**Правильный алгоритм** уже в slash-команде `taskagent-tasks.md`:

```
taskagent_list with project_id = <resolved>, status = ["inbox", "todo", "in_progress"]
Never use status=all — token-heavy
```

Но `/taskagent-tasks` срабатывает только при явном вызове команды, не при
...
> «Skip WorkspaceGraph … when `taskagent_relations` / `taskagent_plan_graph`
> already answer the question»

Агент этого не видел → `workspacegraph_search limit=100` → 170 KB, хотя
`taskagent_list status=active` уже дал ответ.

MCP-описание `taskagent_list` (`crates/mcp/src/tools.rs`) само предлагает
search:

```
prefer `active` or a narrow status filter, or `taskagent_search`
```

**Тройное давление:** policy → taskagent.mdc → MCP tool description — все в
сторону search.
...
| `taskagent.mdc` с `alwaysApply: false` | hygiene мог не подгрузиться; policy (с search-hint) — всегда |
...
Запрос: проверь задачи cloud, закрой
    → alwaysApply: taskagent-policy (hint: use taskagent_search)
    → keyword: "taskagent cloud" → search ×2 + workspacegraph 170KB

Правильный путь (taskagent-tasks.md) — не auto-load
...
**Итог:** инструкции **были написаны частично** (в `/taskagent-tasks`), но
...
1. **Убрать/уточнить** «Prefer search» в `taskagent.mdc` и `taskagent-policy.mdc`:
   - `list` — для «что открыто / аудит / закрыть»
   - `search` — только текстовый поиск по архиву/комментариям по явному запросу

2. **Добавить** секцию или `taskagent-audit.mdc`:
   ```
...
3. **Включить** `workspacegraph.mdc` в `RULE_FILES` + `plugin.json`, с явным:
   > «Never workspacegraph_search to list open tasks — use taskagent_list»

4. **Поправить** description `taskagent_list` в `crates/mcp/src/tools.rs` —
   убрать «or taskagent_search».

5. **Обновить** `docs/README.ru.md` — «bundled» ≠ «installed by default».

6. **`taskagent-cursor doctor`** — предупреждать, если `workspacegraph.mdc`
...
| Файл | Роль |
|------|------|
| `cursor/rules/taskagent-policy.mdc` | alwaysApply; hint search |
| `cursor/rules/taskagent.mdc` | on-demand; Prefer search |
| `cursor/rules/workspacegraph.mdc` | bundled, не installed |
| `cursor/commands/taskagent-tasks.md` | правильный list-workflow |
| `lib/rules.mjs` | RULE_FILES = 2 файла |
| `.taskagent-plugin/plugin.json` | rules manifest без workspacegraph |

...
const HOOK = join(__dirname, "..", "hooks", "user-prompt-submit.mjs");
...
test("detects 'sync' keyword", () => {
  ...
  assert.equal(code, 0);
  assert.match(stdout, /\/taskagent-claude:sync/);
});

test("detects 'status' keyword", () => {
  const { stdout, code } = run("what is the status of the project");
  assert.equal(code, 0);
  assert.match(stdout, /\/taskagent-claude:status/);
});
...
test("detects 'close' keyword", () => {
  ...
  assert.equal(code, 0);
  assert.match(stdout, /\/taskagent-claude:close/);
});

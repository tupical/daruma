...
`--plan` работает только если инструмент `daruma_ai_decompose` реально зарегистрирован на сервере, к которому подключён клиент. Он есть в каталоге SaaS/Meisei, но **OSS-сервер daruma его не регистрирует** — `daruma-claude` перед вызовом проверяет `tools/list`, и если инструмента нет, печатает уведомление `[decompose]` и молча откатывается к исполнению одной корневой задачи — так же, как в случае с отсутствующим API-ключом ниже.

На серверах, где инструмент есть, AI-декомпозиция дополнительно требует, чтобы `OPENAI_API_KEY` был выставлен **на стороне daruma-сервера**. Без него `daruma_ai_decompose` возвращает `502 ai_unavailable`, а `daruma-claude` откатывается тем же образом. Чтобы получить настоящую декомпозицию — экспортни ключ перед запуском сервера:

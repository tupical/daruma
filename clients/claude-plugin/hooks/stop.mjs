...
// reusable lesson via taskagent_comment at session end.
//
// This hook intentionally prints nothing when there is no active taskagent
// session (TASKAGENT_ACTIVE_TASK env var absent), so it doesn't fire on
...
const activeTask = process.env.TASKAGENT_ACTIVE_TASK ?? "";
...
process.stdout.write(
  `[taskagent-claude/auto-record] Active task: ${activeTask}\n` +
  `If this session produced a concrete reusable lesson (command, invariant, bug pattern, file path), ` +
  `capture it now via /taskagent-claude:capture or call taskagent_comment directly:\n` +
  `  taskagent_comment task_id="${activeTask}" body="lesson: <short durable lesson>"\n` +

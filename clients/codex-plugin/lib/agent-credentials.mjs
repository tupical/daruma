// Unified token discovery for taskagent-codex.
//
// Resolution order (same across all taskagent clients):
//   1. TASKAGENT_TOKEN env var             — highest priority; CI / headless
//   2. ~/.agents/taskagent/credentials.json  — written by `pair` commands
//   3. ~/.config/taskagent/credentials.json  — legacy XDG path (auto-migrated)
//   4. Interactive prompt                   — last resort; result is persisted
//
// This module is intentionally a copy of clients/cursor-plugin/lib/agent-credentials.mjs
// with the same public API so both plugins stay in sync. Keep them identical
// except for the module header comment.
...
/**
 * Resolve the token to use for MCP connections.
 *
 * Discovery order:
 *   1. `overrides.token`
 *   2. `TASKAGENT_TOKEN` env var
 *   3. Active profile from credentials file
 *
 * @param {{ token?: string }} [overrides]
 * @returns {Promise<string|null>}
 */
export async function resolveToken(overrides = {}) {
  if (overrides.token) return overrides.token;
  if (process.env.TASKAGENT_TOKEN?.trim()) return process.env.TASKAGENT_TOKEN.trim();
...
/**
 * Build MCP stdio `env` for taskagent-mcp from stored credentials.
 * @param {{ apiUrl?: string, token?: string, workspaceId?: string }} [overrides]
 */
export async function resolveMcpEnvFromCredentials(overrides = {}) {
  ...
  if (overrides.apiUrl) {
    env.TASKAGENT_API_URL = overrides.apiUrl.replace(/\/$/, "");
  }
  if (overrides.token) {
...
export async function resolveHttpProbeUrl(overrides = {}) {
  ...
  return process.env.TASKAGENT_SELFHOST_URL ?? "http://localhost:8080";
}

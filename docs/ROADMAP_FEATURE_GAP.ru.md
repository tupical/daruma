...
Контекст: Daruma — таск-менеджер для совместной работы агентов с минимальным участием пользователя. Ниже — оценка из текущего списка: что уже есть в core, что можно добавлять следующим приоритетом, и как это лучше адаптировать под агентный сценарий.

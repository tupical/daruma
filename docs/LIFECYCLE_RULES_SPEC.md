...
- `blocked` — мутация НЕ выполняется; ошибка с кодом `rule_blocked`,
  телом `{rule_id, rule_key, requirement, outcomes[], unblock[]}`. Сообщение
  блокировки передаётся отдельно как `GateDecision::Blocked.message` и как
  префикс строковой ошибки, а не дублируется в `details`.
  Каждый элемент `unblock` содержит `rule_key`, `requirement_type`, потолок
  переиспользования evidence в `reach` и готовый к передаче в
  `daruma_evidence_submit` объект `evidence: {kind, target?, scope?}`. Для
  требования, которое невозможно удовлетворить до создания сущности, `scope`
  отсутствует, а `note` объясняет необходимость override либо невозможность
  override для правила, которое его запрещает.
- Несколько сработавших правил: `outcomes[]` содержит ВСЕ результаты
  ...
  показали все требования сразу, а не по одному. `unblock[]` выровнен по
  blocked-префиксу `outcomes[]`: элемент с тем же индексом объясняет, как снять
  соответствующую блокировку.

Строковое сообщение ошибки заканчивается суффиксом
` | unblock: <compact JSON array>`. `rule_key` и операторские строки `target`
не запрещают этот разделитель. Парсер начинает с **последнего** вхождения,
пытается разобрать остаток как JSON-массив и при неудаче отступает к предыдущему
вхождению, повторяя попытку до успешного разбора. Это защищает и свободный
`message`, и JSON-суффикс от содержащего ` | unblock: ` операторского текста.
`unblock` остаётся отдельным массивом, а не
полем `outcomes`: `outcomes` уже содержит человеческий `message` той же ошибки
и `rule_id`; вложение потребовало бы либо дублировать `message` в каждой ошибке,

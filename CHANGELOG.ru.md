...
### Планы сохраняют исходный бриф

План теперь может хранить породивший его бриф или промпт в необязательном,
неизменяемом после создания `source_brief`. Значение задаётся при интейке через
`daruma_plan_materialize` или `POST /v1/plans`; раньше место в базе было, но
заполнить его было нельзя.

### Ленивый авто-провижн проекта под репо (по флагу)

Когда MCP-вызов приходит под repo-scope (`scope_path`), у которого нет
привязки в `repo_scopes`, сервер теперь может лениво создать
daruma-проект (title = `basename(scope_path)`) и привязать scope к нему —
так новый репо становится отслеживаемым при первом обращении, без ручного
`daruma_project_use`. По умолчанию выключено (в OSS/self-host поведение
прежнее: непривязанный `scope_path` по-прежнему даёт ошибку); включается
через `DARUMA_AUTO_PROVISION_REPO_PROJECT=1`. Провижн идемпотентный и
защищён от гонок — параллельные первые вызовы сходятся к одному проекту

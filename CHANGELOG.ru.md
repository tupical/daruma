...
оставляем в `git log`. Английский релизный changelog —
[CHANGELOG.md](CHANGELOG.md).

## 2026-08-28 — релиз 0.4.0

Plan-only intake (ADR-0007), lifecycle-правила с evidence-гейтами и
легальными обходами, реестр артефактов, месяц укрепления MCP, AI-infra и
мультиагентной координации. 74 коммита с 0.3.0.

### Заблокированные правила объясняют, как их выполнить

Ошибки `rule_blocked` теперь сохраняют операторское сообщение и добавляют
машиночитаемый JSON-суффикс `unblock` для каждого блокирующего правила:
каждая подсказка называет требуемый evidence, допустимый scope и его
потолок повторного использования; требования, которые невозможно выполнить
до создания сущности, прямо говорят об этом.

### Evidence не действует шире, чем может означать

Один `evidence_submit` в tenant-scope раньше навсегда удовлетворял правило
для всех сущностей тенанта. Теперь у каждого вида evidence есть потолок
читаемости: `acceptance_criteria_defined`, `completion_note` и
`owner_assigned` читаются только в scope своей сущности;
`impact_assessment` и `risk_check_completed` — до плана;
`decision_record` и `artifact_created` — до проекта; `document_read_ack`
остаётся tenant-wide. Запись выше потолка по-прежнему хранится (evidence
неизменяем), но не удовлетворяет ничего.

### `set_status` принимает причину обхода правила

`set_status` теперь принимает `override_reason` (MCP: `daruma_set_status`).
Обход блокирующего правила по-прежнему требует три вещи: `force`, непустую
причину и правило, допускающее обход. Пустая причина не обходит; одно
необходимое правило без обхода блокирует весь переход. Штатный путь мимо
правила — выполнить его через evidence.

### Статус плана обходит допустимую блокировку жизненного цикла

`set_plan_status` несёт тот же контракт `force`/`override_reason` через
команду, MCP-инструмент и HTTP-маршрут. Допустимый обход возвращает
обойдённое правило `PlanBeforeApprove` как предупреждение; пустая причина
и любое необходимое блокирующее правило по-прежнему отклоняют переход.

### `can_start` отвечает и за правила жизненного цикла

`GET /v1/tasks/{id}/can_start` (MCP: `daruma_can_start`) раньше учитывал
только блокировки связей. Теперь он прогоняет тот же lifecycle-гейт, что и
сам переход, и ответ несёт два добавочных поля: `rule_blockers`
(блокирующие правила; непусто ⇒ `ready: false`) и `rule_warnings`
(совещательные правила, на `ready` не влияют). Оба поля опускаются, когда
пусты, — workspace без правил видит прежний ответ.
...
## 2026-07-10 — релиз 0.3.0

### Профили MCP-инструментов

`tools/list` теперь гейтится профилем: новый профиль `default` отдаёт
компактный workflow-набор (33 инструмента), `full` сохраняет полный
каталог (~125). Выбор: `daruma mcp --profile`, `DARUMA_MCP_PROFILE` или
`/v1/mcp?profile=`. **Не задано — теперь `default`**: клиенты, которым
нужны продвинутые инструменты (history, documents, sessions,
workspacegraph, AI-операции, bulk-операции), должны явно включить `full`;
скрытые инструменты возвращают понятную ошибку вместо dispatch. Состав и
миграция — в [docs/mcp/PROFILES.md](docs/mcp/PROFILES.md).

- Каждый инструмент несёт человекочитаемый `title` и MCP-аннотации
  (`readOnlyHint`, `destructiveHint`, `idempotentHint`, `openWorldHint`).
- Внутренний каталог перестроен: каждый инструмент объявляет домен,
  профиль и пресет аннотаций — новый инструмент не может «проскочить».

### Webhook'и task.due

Тик watchdog'а дедлайнов (`DARUMA_DUE_TICK_SECS`, по умолчанию 60 с,
`0` выключает) эмитит событие `task.due`, когда `due_at` активной задачи
проходит — один раз на (задача, дедлайн), с дедупликацией между
рестартами (миграция 0032). Подписки webhook'ов подхватывают его по kind
как любое другое событие; перенос дедлайна перевзводит уведомление.

### Обобщённые fenced-аренды (мультиагентная координация)

`work_leases` обобщены с эксклюзивных path-глобов на аренды ресурсов с
режимами (миграция 0033): `mode` (`exclusive` | `shared_read` | `review`
| `intent` — конфликтуют только записи, intent совещательный),
`target_uri` со схемным матчингом конфликтов и монотонный
`fencing_token` на ресурс, выдаваемый в той же транзакции, что и грант, —
устаревший держатель не пройдёт `check_fencing_token` после перевыдачи.

### Слой WorkUnit (мультиагентная координация)

Задача теперь декомпозируется на **work units** — минимальная единица
диспетчеризации для нескольких агентов на одной задаче:
`POST /v1/work-units` создаёт юниты, `POST /v1/work-units/drain-next`
атомарно клеймит следующий диспетчеризуемый юнит и берёт его аренды в той
же выдаче. Задачи без юнитов не затрагиваются.

### Auto-append в Interview / Human Log

Активность проекта дописывается в автосоздаваемые документы: агентская
активность — в **Interview**, человеческие вехи — в **Human Log**.
Тумблеры на проект (по умолчанию ON) — `GET/PATCH
/v1/projects/{id}/settings` и MCP `daruma_project_settings_get` /
`_update`; изменения event-sourced (миграция 0034). См.
docs/guides/documents-auto-append.md.

### Асинхронные события AI-операций (WS Channel::AiOps)

Серверные AI-операции (`/v1/ai/decompose/{task}`,
`/v1/ai/analyze-complexity/{plan}`) пушат типизированные события
прогресса в новый WS-канал `AiOps` — клиенты рисуют прогресс без
поллинга. HTTP-ответы не изменились.

### Авто-резолв workspace

`POST /v1/workspace-registry/resolve` отображает корень файловой системы
на логический workspace + проект по умолчанию, создавая и привязывая оба
при первом обращении. Новые MCP-инструменты (профиль `full`):
`daruma_workspace_resolve`, `daruma_workspace_list`,
`daruma_project_move_workspace`.

### Корректность планировщика (мультиагентная координация)

Резолвер задач плана теперь учитывает связи `Blocks` между задачами во
всех путях диспетчеризации — `drain_next`, `ready_drain`, `next-task`,
`plan_progress.next_ready` — чтобы параллельные агенты не растаскивали
взаимоблокирующую пару. Резервирование аренд по нескольким целям —
всё-или-ничего в одной транзакции.

### Укрепление AI-промптов

Grounding-контекст (тела задач, комментарии, payload'ы событий),
вставляемый в AI-промпты, теперь огораживается как явно недоверенные
данные с нейтрализацией escape-последовательностей. См.
...
История до 2026-05-20 — см. `git log`. До этого дня формальный changelog
не вёлся.

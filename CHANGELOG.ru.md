...
### Фильтр по статусу для `taskagent_list`

`GET /v1/tasks` и MCP-инструмент `taskagent_list` теперь принимают
параметр `status`. Поддерживается:

- одиночное значение (`inbox`/`todo`/`in_progress`/`in_review`/
  `done`/`cancelled`),
- список через запятую (`status=todo,in_progress`),
- шорткат `status=active` — раскрывается во все нетерминальные
  статусы (`inbox`, `todo`, `in_progress`, `in_review`).

Зачем: когда агент получает «дай оставшиеся задачи», он раньше
тащил весь архив, включая `done`/`cancelled`, и это сильно
захламляло контекст. Теперь типичный сценарий — `taskagent_list`
с `status: "active"`, и в выдачу попадают только живые задачи.
Описание инструмента подсказывает агенту это поведение.

Неизвестный статус → 400. Пустой `status` (после `trim`) считается
ошибкой, чтобы агент не «случайно» получил всё. Фильтрация
выполняется на уровне SQL (`status IN (…)`), а не в памяти.
